...
function analyze(p,first,last,y,m,d,personalExtra=[]){let obvious=0;const low=p.toLowerCase(),personal=[first,last,String(y),String(y).slice(-2),String(m).padStart(2,'0'),String(d).padStart(2,'0'),...personalExtra.filter(Boolean)];personal.forEach(x=>{if(x.length>1&&low.includes(x))obvious++});const unique=new Set(p).size,variety=(/[a-z]/.test(p)?1:0)+(/[A-Z]/.test(p)?1:0)+(/[0-9]/.test(p)?1:0)+(/[^A-Za-z0-9]/.test(p)?1:0);if(p.length<6)return ['BRUH','bruh'];if(obvious>=2&&p.length<12)return ['WEAK','weak'];if(obvious>=1&&p.length<10)return ['MEDIUM','medium'];if(p.length<8)return ['WEAK','weak'];if(variety<2&&p.length<10)return ['MEDIUM','medium'];if(p.length<12||unique<7)return ['STRONG','strong'];if(p.length<16||unique<8||variety<3)return ['VERY STRONG','strong'];if(p.length>=16&&variety>=3&&unique>=10)return ['UNGUESSABLE*','unpredictable'];return ['VERY STRONG','strong']}
